...
from cause_io import Instance, tokens_from_text_and_annotations, despacify
...
def extract():
    ...
    for part in ["train", "test"]:
        ...
        for i, document in enumerate(
            ...
            for clause in document.iterfind(
                ...
                cause = clause.find("{http://www.w3.org/2009/10/emotionml}cause")
                if cause is None:
                    clauses.append([(word, "O") for word in text.split()])
                else:
                    cause_text = despacify(cause.text)
                    cause_text = unfuck(cause_text)
                    clauses.append(tokens_from_text_and_annotations(text, [cause_text]))
    ...
    for identifier in sorted(text_to_info):
        ...
        yield Instance(
            ...
            text=text,
            clauses=clauses,
            tokens=list(chain.from_iterable(clauses)),
        )
