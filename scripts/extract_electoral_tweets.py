...
from cause_io import tokens_from_text_and_annotations, Instance
...
def process_line(line, err_handler=None):
    unitid = line["unitid"]
    text = line["tweet"]
    role_value = line[columnmap["cause"]]
    if not text:
        err_handler.write(f"NOTEXT\t{unitid}\t{text}\t{role_value}\n")
        raise AnnotationError("no text")
    emotions = [emo.strip() for emo in line[columnmap["emotion"]].strip().split(" or ")]
    if not emotions:
        emotions = ["noemo"]
    yield emotions
    yield text
    ...
    if role_value not in text:
        if "," in role_value and all(
            ...
            annotations = []
            if err_handler:
                err_handler.write(f"NOANNOTATION\t{unitid}\t{text}\t{role_value}\n")
    ...
    for batch in "12":
        with open(
            ...
            with open("workdata/errors-et.tsv", "w") as err:
                for line in reader:
                    try:
                        processor = process_line(line, err_handler=err)
                        emotions = next(processor)
                        text = next(processor)
                        extra = next(processor)
                        tokens = next(processor)
                        if line["tweet"] is None:
                            continue
                        yield Instance(
                            text=text, emotions=emotions, tokens=tokens, extra=extra
                        )
